...
(function (global, factory) {
    ...
    function parseOptions(options, args = []) {
        try {
            return options
                ? util.startsWith(options, '{')
                    ? JSON.parse(options)
                    : args.length && !util.includes(options, ':')
                    ? { [args[0]]: options }
    ...
    var Togglable = {
        ...
        methods: {
            ...
            isToggled(el = this.$el) {
                el = util.toNode(el);
                return util.hasClass(el, this.clsEnter)
                    ? true
                    : util.hasClass(el, this.clsLeave)
                    ? false
                    : this.cls
                    ? util.hasClass(el, this.cls.split(' ')[0])
                    : util.isVisible(el);
            },
            ...
            _toggle(el, toggled) {
                ...
                util.$$('[autofocus]', el).some((el) => (util.isVisible(el) ? el.focus() || true : el.blur()));
    ...
    function toggleAnimation(el, show, cmp) {
        ...
        return util.Animation.out(el, animation[1] || animation[0], duration, cmp.origin).then(() => _toggle(el, false),
        );
    }
        ...
        events: [
            {
                ...
                handler(e) {
                    ...
                    if (
                        !defaultPrevented &&
                        hash &&
                        util.isSameSiteAnchor(current) &&
                        !util.within(hash, this.$el) &&
                        util.$(hash, document.body)
                    ) {
                        this.hide();
    ...
    function animate(el, show, { transitionElement, _toggle }) {
        return new Promise((resolve, reject) => util.once(el, 'show hide', () => {
                el._reject?.();
                el._reject = reject;
    ...
    function toMs(time) {
        return time ? (util.endsWith(time, 'ms') ? util.toFloat(time) : util.toFloat(time) * 1000) : 0;
    }
    ...
    function listenForBackgroundClose(modal) {
        return util.on(document, util.pointerDown, ({ target }) => {
            if (
                util.last(active) !== modal ||
                (modal.overlay && !util.within(target, modal.$el)) ||
                util.within(target, modal.panel)
            ) {
                return;
            }
    ...
    function Transitioner(prev, next, dir, { animation, easing }) {
        ...
        return {
            ...
            translate(percent) {
                ...
                util.css(next, props[1]);
                util.css(prev, props[0]);
            ...
            startAutoplay() {
                ...
                this.interval = setInterval(() => {
                    if (
                        !(
                            this.stack.length ||
                            (this.draggable && util.matches(this.$el, ':focus-within')) ||
                            (this.pauseOnHover && util.matches(this.$el, ':hover'))
                        )
                    ) {
                        this.show('next');
                    }
        ...
        events: [
            {
                ...
                handler(e) {
                    if (
                        !this.draggable ||
                        (!util.isTouch(e) && hasSelectableText(e.target)) ||
                        util.closest(e.target, util.selInput) ||
                        e.button > 0 ||
                        this.length < 2
                    ) {
                        return;
            ...
            move(e) {
                ...
                if (this.dragging) {
                    ...
                    if (this.index === this.prevIndex) {
                        ...
                        const dirChange =
                            (util.isRtl ? this.dir * (util.isRtl ? 1 : -1) : this.dir) < 0 ===
                            this.prevPos > this.pos;
    ...
    function hasSelectableText(el) {
        return (
            util.css(el, 'userSelect') !== 'none' &&
            util.toArray(el.childNodes).some((el) => el.nodeType === 3 && el.textContent.trim())
        );
    }
    ...
    var SliderNav = {
        ...
        watch: {
            ...
            navChildren(children) {
                util.attr(children, 'role', 'presentation');
            },
            ...
            slides(slides) {
                slides.forEach((slide, i) => util.attr(slide, {
        ...
        events: [
            {
                ...
                handler(e) {
                    if (
                        util.closest(e.target, 'a,button') &&
    ...
    var Slider = {
        ...
        methods: {
            show(index, force = false) {
                ...
                if (
                    (prev && !util.trigger(prev, 'beforeitemhide', [this])) ||
                    !util.trigger(next, 'beforeitemshow', [this, prev])
                ) {
    ...
    function toItem(el) {
        ...
        for (const attr of ['href', 'caption', 'type', 'poster', 'alt', 'attrs']) {
            item[attr === 'href' ? 'source' : attr] = util.data(el, attr);
        }
