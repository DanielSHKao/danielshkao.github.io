...
(function (global, factory) {
    ...
    function toScrollTargets(elements) {
        return uikitUtil.toNodes(elements).map((node) => {
            ...
            const parent = uikitUtil.scrollParent(node, true);
        ...
        methods: {
            ...
            getCss(percent) {
                ...
                for (const prop in this.props) {
                    this.props[prop](css, uikitUtil.clamp(percent));
                }
                css.willChange = Object.keys(css)
                    .filter((key) => css[key] !== '')
                    .map(uikitUtil.propName)
                    .join(',');
                return css;
            },
        },
    ...
    function transformFn(prop, el, stops) {
        ...
        if (prop === 'x' || prop === 'y') {
            ...
            unit = '';
            transformFn = (stop) => getUnit([stop]) ? uikitUtil.toPx(stop, 'width', el, true) / el.offsetWidth : uikitUtil.toFloat(stop);
        }
    ...
    function parseStops(stops, fn = uikitUtil.toFloat) {
        ...
        for (let i = 0; i < length; i++) {
            let [value, percent] = uikitUtil.isString(stops[i])
                ? stops[i].trim().split(/ (?![^(]*\))/)
                : [stops[i]];
            value = fn(value);
