...
(function (global, factory) {
        ...
        return intersection({
            handler: function (entries, observer) {
                ...
                for (const el of util.toNodes(util.isFunction(targets) ? targets(this) : targets)) {
                    util.$$('[loading="lazy"]', el)
                        .slice(0, preload - 1)
                        .forEach((el) => util.removeAttr(el, 'loading'));
                }
    ...
    function Transitioner (prev, next, dir, { center, easing, list }) {
        const from = prev
            ...
            : getLeft(next, list, center) + util.dimensions(next).width * dir;
        const to = next
            ? getLeft(next, list, center)
            : from + util.dimensions(prev).width * dir * (util.isRtl ? -1 : 1);
    ...
    function getElLeft(el, list) {
        return (
            (el &&
                (util.position(el).left + (util.isRtl ? util.dimensions(el).width - util.dimensions(list).width : 0)) *
                    (util.isRtl ? -1 : 1)) ||
            0
        );
    }
    ...
    var Component = {
            ...
            sets({ sets: enabled }) {
                ...
                for (let i = 0; i < this.length; i++) {
                    ...
                    if (this.center) {
                        if (
                            left < width / 2 &&
                            left + slideWidth + util.dimensions(util.getIndex(+i + 1, this.slides)).width / 2 > width / 2
                        ) {
                            sets.push(+i);
                ...
                for (const el of this.navItems) {
                    ...
                    if (index !== false) {
                        el.hidden =
                            !this.maxIndex ||
                            index > this.maxIndex ||
                            (this.sets && !util.includes(this.sets, index));
                    }
                }
        ...
        events: {
            beforeitemshow(e) {
                if (
                    ...
                    this.sets &&
                    this.stack.length < 2 &&
                    !util.includes(this.sets, this.index)
                ) {
                ...
                this.duration =
                    ...
                    (util.dimensions(this.slides[index]).width / this.avgWidth);
        ...
        methods: {
            reorder() {
                ...
                this.slides.forEach((slide, i) => util.css(
                        slide,
                        'order',
        ...
        for (const index in slides) {
            ...
            for (const i of [-1, 1]) {
                ...
                diff = Math.max(
                    diff,
                    slideWidth / 2 +
                        util.dimensions(slides[util.getIndex(+index + i, slides)]).width / 2 -
                        (left - listHalf),
                );
            }
            ...
            if (
                diff > util.sumBy(
                    ...
                    (slide) => util.dimensions(slide).width,
                )
            ) {
                return true;
            }
